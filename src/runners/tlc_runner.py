...
class TLC_KLCommRunner:
    ...
    def get_comm_matrix(self, KL):
        comm_agents = KL <= torch.mean(KL)
        n_agnets = len(comm_agents)
        comm_agents = np.array(comm_agents, dtype=int)
        mat = np.zeros([n_agnets,n_agnets])
        for i in range(n_agnets):
    ...
    def run(self, test_mode=False):
        ...
        while not terminated:
            ...
            if first_step:
                ...
                last_obs_norm = F.softmax(torch.tensor(last_obs), dim=1)
                obs_norm = F.softmax(torch.tensor(obs), dim=1)
                KL = F.kl_div(torch.log(obs_norm), last_obs_norm, reduction='none').sum(dim=1)
                comm_mat = self.get_comm_matrix(KL)
                adj = comm_mat + np.eye(self.args.n_agents)
